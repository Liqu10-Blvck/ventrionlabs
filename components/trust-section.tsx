import { CheckCircle2, FileText, GitBranch, Shield } from "lucide-react"

const trustIcons = [FileText, GitBranch, Shield, CheckCircle2] as const

type TrustSectionProps = {
  eyebrow: string
  title: string
  description: string
  items: Array<{ title: string; description: string }>
}

export function TrustSection({ eyebrow, title, description, items }: TrustSectionProps) {
  const trustSignals = items.map((item, index) => ({
    ...item,
    icon: trustIcons[index] ?? FileText,
  }))

  return (
    <section id="why-ventrion" className="px-6 py-20 sm:py-24 md:py-32">
      <div className="mx-auto max-w-6xl">
        <div className="mb-10 flex flex-col items-center text-center sm:mb-16">
          <p className="mb-3 text-sm font-medium uppercase tracking-[0.24em] text-muted-foreground dark:text-white/45">
            {eyebrow}
          </p>
          <h2 className="max-w-4xl text-balance text-3xl font-semibold tracking-tight text-foreground dark:text-white sm:text-4xl md:text-5xl">
            {title}
          </h2>
          <p className="mt-4 max-w-2xl text-sm leading-relaxed text-muted-foreground dark:text-white/60 sm:text-base">
            {description}
          </p>
        </div>

        <div className="grid gap-4 sm:gap-6 sm:grid-cols-2 lg:grid-cols-4">
          {trustSignals.map((signal, index) => {
            const Icon = signal.icon
            return (
            <div
              key={signal.title}
              className="futuristic-panel motion-safe-lift rounded-2xl border border-border bg-card/80 px-5 py-6 shadow-sm transition-all duration-300 hover:border-foreground/15 hover:bg-card hover:shadow-2xl hover:shadow-black/8 dark:border-white/10 dark:bg-white/3 dark:hover:border-white/20 dark:hover:bg-white/5 dark:hover:shadow-black/20"
              style={{ animationDelay: `${index * 110}ms` }}
            >
              <div className="mb-4 flex size-10 items-center justify-center rounded-lg border border-border bg-secondary text-foreground dark:border-white/10 dark:bg-white/4 dark:text-white">
                <Icon className="size-5" />
              </div>
              <h3 className="text-base font-semibold text-foreground dark:text-white">{signal.title}</h3>
              <p className="mt-2 text-sm leading-relaxed text-muted-foreground dark:text-white/60">
                {signal.description}
              </p>
            </div>
          )})}
        </div>
      </div>
    </section>
  )
}
